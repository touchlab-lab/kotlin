        ...
        referenceTargetSymbols.flatMap { symbol ->
            when (symbol) {
                is KtFirSyntheticJavaPropertySymbol ->
                    when (expression.readWriteAccess(false)) {
                        ReferenceAccess.READ -> listOfNotNull(symbol.getter.psi)
                        ReferenceAccess.WRITE -> listOfNotNull(symbol.setter?.psi)
                        ReferenceAccess.READ_WRITE -> listOfNotNull(symbol.getter.psi, symbol.setter?.psi)
                    }
